...
for (f in unique((monocyte_data %>% filter(Exp=="LPCwash"))$fname)) {
  ...
  model<-simulate.random(dist0, fus_num, B=100)
...
```{r}
  ...
  # syn_num=nrow(test_data)
  # nuc_num=fus_num+syn_num
  my_plot<-ggplot(all_data %>% filter(Exp!="LPCon"), aes(x=Nuclei, color=Exp))+stat_ecdf()+coord_cartesian(xlim=c(0,25))
  
  dist0<-initial_dist(all_data %>% filter(Exp=="30min"), max(fnum$fuse_num)-min(fnum$fuse_num),nuc_num90)
  model<-bind_rows(lapply(c(fus_num60, fus_num90), function(x) simulate.random(dist0, x)))
  ...
  dist0<-initial_dist(all_data %>% filter(Exp=="30min"), max(fnum$fuse_num)-min(fnum$fuse_num), (max(fnum$syn_num)-min(fnum$syn_num)))
  model<-bind_rows(lapply((fnum %>% filter(Exp != "LPCon"))$fuse_num, function(x)      simulate.founder(dist0, x, weights = 1:length(dist0))))
  my_plot+geom_line(data=model %>% filter(Nuclei>1) %>% group_by(fusNum) %>% mutate(freq=dist.to.cdf(N)), aes(x=Nuclei, y=freq, alpha=factor(fusNum)),col='red')+scale_alpha_discrete(range=c(0.2,1), guide=F)
  ggsave("../Simulation/founder0.png")
  
  dist0<-initial_dist(all_data %>% filter(Exp=="30min"), max(fnum$fuse_num)-min(fnum$fuse_num),1.6*(max(fnum$syn_num)-min(fnum$syn_num)))
  model<-bind_rows(lapply((fnum %>% filter(Exp != "LPCon"))$fuse_num, function(x)      simulate.founder(dist0, x, weights = 1:length(dist0))))
  my_plot+geom_line(data=model %>% filter(Nuclei>1) %>% group_by(fusNum) %>% mutate(freq=dist.to.cdf(N)), aes(x=Nuclei, y=freq, alpha=factor(fusNum)),col='red')+scale_alpha_discrete(range=c(0.2,1), guide=F)
  ggsave("../Simulation/founder1.png")
  
  dist0<-initial_dist(all_data %>% filter(Exp=="30min"), max(fnum$fuse_num)-min(fnum$fuse_num),1.6*(max(fnum$syn_num)-min(fnum$syn_num)))
  dist1<-initial_dist(all_data %>% filter(Exp=="30min"), max(fnum$fuse_num)-min(fnum$fuse_num),7500)
  model<-bind_rows(lapply((fnum %>% filter(Exp != "LPCon"))$fuse_num, function(x)      simulate.founder2(dist0, dist1, x, weights = 1:length(dist0))))
  my_plot+geom_line(data=model %>% filter(Nuclei>1) %>% group_by(fusNum) %>% mutate(freq=dist.to.cdf(N)), aes(x=Nuclei, y=freq, alpha=factor(fusNum)),col='red')+scale_alpha_discrete(range=c(0.2,1), guide=F)
...
boot_int<-function(data, stat, R, type='perc', conf=0.95) {
  ...
  df$y = stat(data, NULL)
  df[2:3]<-boot.ci(tmp_boot, conf=conf, type='bca')$bca[1, 4:5]
  return(df)
}
