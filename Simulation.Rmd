...
```{r}
  ...
  dist0<-initial_dist1(all_data %>% filter(Exp=="30min"), max(fnum$fuse_num)-min(fnum$fuse_num),nuc_num90)
  model<-simulate.random(dist0, c(fus_num60,fus_num90))
  ...
  dist0<-initial_dist1(all_data %>% filter(Exp=="30min"), max(fnum$fuse_num)-min(fnum$fuse_num), (max(fnum$syn_num)-min(fnum$syn_num)))
  model<-simulate.founder(dist0, fnum$fuse_num, weights = 1:length(dist0))
  my_plot+geom_line(data=model %>% filter(Nuclei>1) %>% group_by(fusNum) %>% mutate(freq=dist.to.cdf(N)), aes(x=Nuclei, y=freq, alpha=factor(fusNum)),col='red')+scale_alpha_discrete(range=c(0.2,1), guide=F)
  # ggsave("../Simulation/founder0.png")
  
  dist0<-initial_dist1(all_data %>% filter(Exp=="30min"), max(fnum$fuse_num)-min(fnum$fuse_num),1.5*(max(fnum$syn_num)-min(fnum$syn_num)))
  model<-simulate.founder(dist0, fnum$fuse_num, weights = function(x) return(x*(1:length(x))), B=100)
